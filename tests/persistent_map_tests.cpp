...
int main() {
    ...
    if (a.size() != 1000) {
        std::printf("PERSIST FAIL: a.size=%zu\n", a.size());
        ++fails;
    }
    if (b.size() != 1000) {
        std::printf("PERSIST FAIL: b.size=%zu\n", b.size());
        ++fails;
    }
    ...
    if (sa.size() != 1000) {
        std::printf("SET PERSIST FAIL: sa.size=%zu\n", sa.size());
        ++fails_set;
    }
    if (sb.size() != 1) {
        std::printf("SET PERSIST FAIL: sb.size=%zu\n", sb.size());
        ++fails_set;
    }
    ...
    }
    {
        ...
        if (ca.size() != 90) {
            std::printf("CLASH PERSIST FAIL: ca.size=%zu\n", ca.size());
            ++fails_clash;
        }
        if (cb.size() != 45) {
            std::printf("CLASH PERSIST FAIL: cb.size=%zu\n", cb.size());
            ++fails_clash;
        }
    }
